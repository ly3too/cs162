...
void process_argv_init(process_t *proc)
{
	proc->argc = 0;
	proc->argv = NULL;
}
...
void process_set_cmd(process_t *proc, char *cmd)
{
	proc -> cmd = cmd;
}

void process_push_arg(process_t *proc, char * arg)
{
	proc->argc += 1;
...
void process_set_input(process_t *proc, int fd)
{
...
void process_set_output(process_t *proc, int fd)
{
...
void process_clean_up(process_t *proc)
{
	free (proc -> cmd);
...
void job_process_vecotr_push (job_t *job_p, process_t *proc)
{
...
void job_process_vecotr_init(job_t *job_p)
{
	job_p -> size = 0;
	job_p -> processv = NULL;
}

void job_init(job_t *job_p)
{
...
void job_append_arg(job_t *job_p, const char *arg)
{
...
void job_set_bg_true(job_t *job_p)
{
	job_p -> bg = true;
}

void job_clean_up(job_t *job_p)
{
	free (job_p -> cmd_line);
...
void job_vector_push(job_vector_t *job_vect, job_t *job)
{
...
void job_vector_init(job_vector_t *job_vect)
{
	job_vect -> size = 0;
	job_vect -> jobv = NULL;
}
...
job_t * jov_vector_get_job(job_vector_t *job_vect, size_t n)
{
...
void destroy_job_vector (job_vector_t * job_vect)
{
	;
}
